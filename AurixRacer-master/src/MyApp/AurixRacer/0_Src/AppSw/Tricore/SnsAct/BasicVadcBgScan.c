...
{
	IR_InfraRed.ObstacleFlag = FALSE;
    /* VADC Configuration */
...
void BasicVadcBgScan_run(void)
{
	...
	if(IR_AdcResult[0] > 1000)	IR_InfraRed.ObstacleFlag=TRUE;
	else	IR_InfraRed.ObstacleFlag=FALSE;
	if(IR_AdcResult[0] > 980)	IR_InfraRed.LowObstacleFlag=TRUE;
		else	IR_InfraRed.LowObstacleFlag=FALSE;
}
