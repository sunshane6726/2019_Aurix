...
void GtmTomPwmHl_run(void){
    ...
    if(IR_Ctrl.SpeedFlag == TRUE){
    	if(IR_InfraRed.ObstacleFlag == FALSE){
    		if(IR_Ctrl.rad <0) vol = 0.35 + (IR_Ctrl.rad/6);
    		else vol = 0.35 - (IR_Ctrl.rad/6);
    	}
    	else{
    		if(IR_Encoder.StopFlag == FALSE)	vol = -0.4;
    		else{
    			if(IR_Encoder.StopFlag == FALSE)
    				vol = -0.2;
    			else	vol=0.0;
    		}
    	}
    	...
    	/*if(IR_Encoder.speed<50){
    		IR_Ctrl.GolVol += 0.01;
    	}
    	else if(IR_Encoder.speed > 100){
    		IR_Ctrl.GolVol -= 0.01;
    	}
